...
async function startServer(): Promise<void> {
  ...
  app.get('/health', (_req, res) => {
    res.json(successResponse({ status: 'healthy', service: 'invoice-service' }));
  });
  ...
  const server = app.listen(config.port, () => {
    logger.info(`Invoice service listening on port ${config.port}`);
  });
...
startServer().catch((err) => {
  logger.error({ err }, 'Failed to start invoice service');
  process.exit(1);
});
